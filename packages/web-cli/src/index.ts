...
//   ├── bundled-tomny-core/<plat-arch>/tomny-core[.exe]
//   └── static/                  ← SPA assets
//
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const bundled = path.join(cliRoot, 'bundled-tomny-core', platArch, BACKEND_BINARY);
  return bundled;
}
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: build or install Tomny Core and set AIONUI_BACKEND_BIN.');
    console.warn('');
