...
describe('backtest', () => {
  it('produces consistent metrics on a trending series', () => {
    ...
    const res = backtest('AAPL', bars(closes), 'sma_crossover', { fastPeriod: 5, slowPeriod: 20 }, { maxPositionPct: 0.5, stopLossPct: 0, takeProfitPct: 0 }, 10_000);
  ...
  it('respects stop-loss on a crash after entry', () => {
    ...
    const res = backtest('AAPL', bars([...up, ...crash]), 'sma_crossover', { fastPeriod: 5, slowPeriod: 20 }, { maxPositionPct: 1, stopLossPct: 0.05, takeProfitPct: 0 }, 10_000);
