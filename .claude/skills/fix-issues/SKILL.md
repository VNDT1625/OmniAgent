...
```bash
gh api repos/VNDT1625/OmniAgent/issues/<number>/comments \
...
```bash
gh issue list --repo VNDT1625/OmniAgent --state open --label bug --limit 50 \
...
gh api repos/VNDT1625/OmniAgent/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo VNDT1625/OmniAgent --body "$(cat <<'EOF'
...
```bash
gh pr list --repo VNDT1625/OmniAgent --state open --search "<error-keyword-or-file>" --json number,title
```
...
| --------- | ------------------ | ------------------------------------------------------------------- |
| repo      | VNDT1625/OmniAgent | GitHub repository (owner/repo)                                      |
