...
<p align="center">
  <a href="../../readme.md">English</a> | <strong>简体中文</strong> | <a href="./readme_tw.md">繁體中文</a> | <a href="./readme_jp.md">日本語</a> | <a href="./readme_ko.md">한국어</a> | <a href="./readme_es.md">Español</a> | <a href="./readme_pt.md">Português</a> | <a href="./readme_tr.md">Türkçe</a> | <a href="./readme_ru.md">Русский</a> | <a href="./readme_uk.md">Українська</a> | <a href="https://www.omni.com" target="_blank">官方网站</a>
</p>

<p align="center">
  <strong>💬 社区：</strong> <a href="https://discord.gg/2QAwJn7Egx" target="_blank">Discord (English)</a> | <a href="../../resources/wx-9.png" target="_blank">微信 (中文群)</a> | <a href="https://twitter.com/Omni" target="_blank">Twitter</a>
</p>
...
[✨ Cowork 演示](#-cowork-演示) ·
[🤔 为什么选择 Omni？](#-为什么选择-omni-而非-claude-cowork) ·
[🚀 快速开始](#-快速开始) ·
[💬 社区](#-社区与支持)
...
**Omni 不只是个聊天工具。** 它是一个真正的 Cowork 平台，AI Agent 就像您的得力助手，在电脑上帮您处理各种任务——读文件、写代码、查资料、自动化工作流。Agent 的一举一动都在您的掌控之中，透明可见。

|                       | 传统 AI 聊天客户端 | **Omni (Cowork)**                                                                                      |
| :-------------------- | :----------------- | :----------------------------------------------------------------------------------------------------- |
...
Omni 自带完整的 AI Agent。不像其他工具需要你手动安装各种 CLI 工具，**Omni 装好就能用，开箱即用**。
...
想把文档/表格直接交给 Agent？Omni 内置 **[OfficeCLI](https://github.com/VNDT1625/OfficeCli)**，让 PPT（Morph 转场）、Word（`.docx`）与 Excel（`.xlsx/.xlsm/.csv`）从需求到可交付结果更高效、更稳定。
三类助手对应各自的文件工作流：生成的成稿可直接编辑、可复用。

#### **PPT 助手**

> **输出：可编辑 Morph PPT（`.pptx`）**
> 页间转场连贯、风格统一；底层由 [OfficeCLI](https://github.com/VNDT1625/OfficeCli) 驱动。
...
> **输出：可编辑 Word（`.docx`）**
> 支持论文/写作的结构、段落与格式组织；底层由 [OfficeCLI](https://github.com/VNDT1625/OfficeCli) 驱动。
...
> 用 `xlsx` 生成/更新表格，自动美化并完成数据分析；底层由 [OfficeCLI](https://github.com/VNDT1625/OfficeCli) 驱动。
...
如果你已经在用 Claude Code、Codex、Hermes Agent 或 OpenClaw，Omni 会自动发现它们，让你同时和这些 Agent 一起协作——当然，还有内置 Agent。

**支持的 Agent：** 内置 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Tomni Agentic（aionrs，Omni 随附的 Rust 后端服务） • Snow CLI • Hermes Agent • Cursor Agent 等
...
- **多 Agent 并行执行** — Leader 将任务分解为子任务并委派给并行运行的 Teammate Agent；每个 Teammate 通过 ACP（Agent Communication Protocol，Omni 的多 Agent 协调层）、Gemini 或 Aionrs 使用独立模型
- **Leader 统筹编排** — Leader 分配、追踪并汇总结果；支持的后端包括 Claude Code、Codex、Hermes Agent、Gemini、Snow CLI 和 Tomni Agentic
...
- **支持的后端** — Claude Code、Codex、Gemini、Snow CLI、Tomni Agentic（aionrs）；其他具备 `mcpCapabilities.stdio` 的 ACP 后端自动支持
...
其他 AI 应用可能只给你个聊天窗口，**但 Omni 给你的是完整的 Cowork Agent**。
...
| AWS Bedrock 凭证                         | 通过 Tomni Agentic（aionrs）的 Bedrock 驱动 Agent |
| Ollama / LM Studio（本地）               | 本地模型 Cowork Agent                             |
| NewAPI 网关                              | 统一访问 20+ 模型                                 |

不管用哪个模型，Agent 的能力都一样强大——文件读写、网络搜索、图像生成、工具调用，一个不少。Omni 支持 **30+ 个 AI 平台**，云端本地都能用。
...
Omni 还支持 [NewAPI](https://github.com/QuantumNous/new-api) 网关服务 — 一个统一的 AI 模型中心，聚合和分发各种大语言模型。在同一界面中灵活切换不同模型，满足各种任务需求。
...
- **三层技能体系** — 内置技能（随 Omni 附带）、自定义技能（你自己的）以及扩展技能（第三方扩展贡献）；通过技能指示器按对话启用/禁用
...
Omni 内置 **21 个专业助手**，每个都有独特能力，还能通过自定义技能继续扩展：
...
**自定义技能**：在 `skills/` 目录下创建你的专属技能，随时为助手开启或关闭，让 AI 能力无限扩展。技能来源分三层：内置（随 Omni 附带）、自定义（你自己的）以及扩展（通过扩展 SDK 贡献）。内置技能有 `pptx`、`docx`、`pdf`、`xlsx`、`mermaid` 等。
...
> **设置：** Omni 设置 → WebUI 设置 → Channel，配置 Bot Token。
...
> [远程互联网访问教程](https://github.com/VNDT1625/Omni/wiki/Remote-Internet-Access-Guide-Chinese)
...
- **防休眠** — Omni 会自动阻止系统休眠，任务激活期间检测唤醒后的漏触发
...
> [图像生成模型配置指南](https://github.com/VNDT1625/Omni/wiki/Omni-Image-Generation-Tool-Model-Configuration-Guide)
...
Omni 是一个**免费开源的 Multi-AI Agent 桌面应用**。相比只能在 macOS 上用、还只能绑定 Claude 的 Claude Cowork，Omni 支持全模型、跨平台，是它的全面升级版。

| 维度     | Claude Cowork | Omni                                                   |
| :------- | :------------ | :----------------------------------------------------- |
...
答：<strong>完全不需要。</strong> Omni 自带 AI Agent，装好就能用。用 Google 登录或者输入任意 API 密钥就行。如果你已经装了 Claude Code 或 Gemini CLI 这些 CLI 工具，Omni 会自动发现并集成它们，功能更强大。
</details>

<details>
<summary><strong>问：我可以用 Omni 做什么？</strong></summary>
答：Omni 就是你的<strong>私有 Cowork 工作空间</strong>。内置 Agent 可以帮你批量整理文件夹、处理 Excel 数据、生成文档、搜索网络、生成图像。通过多 Agent 模式，你还能在同一界面同时使用 Claude Code、Codex 和其他强大的 CLI Agent。
</details>

<details>
<summary><strong>问：它是免费的吗？</strong></summary>
答：Omni 完全免费且开源。你可以用 Google 登录免费使用 Gemini，或者用任何你喜欢的 API 密钥。
</details>
...
<p align="center">
  <em>Julian Goldie SEO — Hermes + Omni is Insane (FREE!) · 2.7万次观看</em> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <em>Julian Goldie SEO — OpenClaw + Omni is Insane (FREE!) · 1.1万次观看</em>
</p>
...
> **制作了关于 Omni 的视频？** [在 X 上告诉我们](https://x.com/Omni)，我们会在这里展示！
...
- [📖 完整安装指南](https://github.com/VNDT1625/Omni/wiki/Getting-Started) — 从下载到配置，一步步教你
- [⚙️ LLM 配置指南](https://github.com/VNDT1625/Omni/wiki/LLM-Configuration) — 多平台 AI 模型怎么配置
- [🤖 多 Agent 模式设置](https://github.com/VNDT1625/Omni/wiki/ACP-Setup) — 把终端 AI Agent 集成进来
- [🔌 MCP 工具配置](https://github.com/VNDT1625/Omni/wiki/MCP-Configuration-Guide) — 模型上下文协议服务器设置
- [🌐 WebUI 配置指南](https://github.com/VNDT1625/Omni/wiki/WebUI-Configuration-Guide) — WebUI 完整设置教程

**🎯 使用场景**

- [📁 文件管理](https://github.com/VNDT1625/Omni/wiki/file-management) — 让 AI 帮你整理文件
- [📊 Excel 处理](https://github.com/VNDT1625/Omni/wiki/excel-processing) — AI 驱动的数据处理
- [🎨 图像生成](https://github.com/VNDT1625/Omni/wiki/Omni-Image-Generation-Tool-Model-Configuration-Guide) — AI 图像生成
- [📚 更多使用场景](https://github.com/VNDT1625/Omni/wiki/Use-Cases-Overview)

**❓ 支持与帮助**

- [❓ FAQ](https://github.com/VNDT1625/Omni/wiki/FAQ) — 常见问题和解决方案
- [🔧 配置与使用教程](https://github.com/VNDT1625/Omni/wiki/Configuration-Guides) — 完整配置文档
...
- [报告问题](https://github.com/VNDT1625/Omni/issues) — 遇到 bug 或有新功能想法？告诉我们
- [发布更新](https://github.com/VNDT1625/Omni/releases) — 获取最新版本
...
<td>
  <a href="https://packycode.com" target="_blank">PackyCode</a> 是一家可靠高效的 API 中继服务提供商，为 Claude Code、Codex、Gemini 等平台提供中继服务。感谢 PackyCode 为支持 Omni 用户低成本使用，为我们的用户提供专属 <a href="https://www.packyapi.com/register?aff=omni" target="_blank">9折优惠</a>，付款时使用优惠码 <code>omni</code> 可立减 10%。
</td>
</tr>
<tr>
<td width="170" align="center">
  <a href="https://atomgit.com/VNDT1625/Omni" target="_blank">AtomGit</a>
</td>
<td>
  <a href="https://atomgit.com/VNDT1625/Omni" target="_blank">AtomGit</a> — 面向全球开发者的开源社区与代码托管平台。Omni 已在 AtomGit 上线，欢迎访问。
</td>
</tr>
</table>
...
[报告 Bug](https://github.com/VNDT1625/Omni/issues) · [请求功能](https://github.com/VNDT1625/Omni/issues)
