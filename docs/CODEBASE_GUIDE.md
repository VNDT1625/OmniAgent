...
> `aionui.company.roleConversations`. (2) **Agent không biết mình là ai**: `openRoleChat` cũ chỉ inject
...
29. [Spec đang mở: OmniAgent Enhancements](#29-spec-đang-mở-omniagent-enhancements)
...
(B) bộ chức năng OmniAgent/Studio/Manager mở rộng (mỗi cái có route + bridge + i18n module riêng,
...
#### B. Chức năng OmniAgent / Studio / Manager (mở rộng)
...
├── bridge/                     ← IPC handlers (Electron-native + OmniAgent registration)
│   ├── index.ts                ← initAllBridges(): đăng ký TẤT CẢ bridges (Electron-native + OmniAgent)
...
│   └── petTypes.ts
│
│   ── OmniAgent modules (Main-process, KHÔNG đụng aioncore Rust) ──
...
> **Lưu ý wiring:** Các OmniAgent/Manager bridge KHÔNG nằm trong `process/bridge/`; chúng sống trong
...
> Các trang OmniAgent dùng đường dẫn `/settings/*` nhưng **component nằm ngoài** `pages/settings/`:
> Company → `pages/company/`, Browser → `pages/browser/`, Testing → `pages/testing/`,
> Monitor → `pages/monitor/` (xem "OmniAgent Pages" ở trên).
...
### OmniAgent Pages (desktop-only)

Các page mới phục vụ spec OmniAgent (xem mục 29). Mỗi page là một `pages/<feature>/` với `index.tsx`
...
> **Tổng cộng 9 ngôn ngữ.** Tiếng Việt (`vi-VN`) đã được thêm (Yêu cầu 4 của OmniAgent spec) — đã có
...
> 33 module (xem `packages/desktop/src/common/config/i18n-config.json`). Các module OmniAgent/Manager: `resource`, `company`, `browser`,
...
Mistral Vibe, Nanobot, Aion CLI (aionrs), Snow CLI, Hermes Agent, Cursor Agent...
...
```
User → Leader Agent (Claude Code / Codex / Gemini / Aion CLI)
         ↓ chia task
    ┌────┴────┐
...
Giống mọi năng lực OmniAgent khác, Scheduled Tasks phơi **hai mặt phẳng**, cùng dùng aioncore
...
## 29. Spec đang mở: OmniAgent Enhancements

File: `.kiro/specs/aionui-enhancements/requirements.md`

Đây là spec phát triển **OmniAgent** — fork của AionUi cho nhu cầu cá nhân.
...
**Note**, **Schedule**, có AI hỗ trợ. Độc lập với spec OmniAgent (mục 29), **không đụng aioncore**.
