...
# Preserve blockmaps generated by electron-builder. electron-updater uses the
# Windows installer blockmap to download only changed package blocks.
mapfile -t BLOCKMAPS < <(find "$ARTIFACTS_DIR" -type f -name "*.blockmap" | sort)
for file in "${BLOCKMAPS[@]}"; do
  cp -f "$file" "$OUTPUT_DIR/"
done
...
for installer in "$OUTPUT_DIR"/*.exe; do
  ...
  if [ ! -f "${installer}.blockmap" ]; then
    echo "::error::Missing differential update blockmap for $(basename "$installer")"
    exit 1
  fi
done
