...
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/VNDT1625/OmniAgent/main/scripts/install-web.sh | bash
#   # Or specify version:
...
MIRROR="${MIRROR:-https://github.com/VNDT1625/OmniAgent/releases/download}"
...
resolve_version() {
    ...
    if [[ "$VERSION" == "latest" || "$VERSION" =~ [a-zA-Z_] ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/VNDT1625/OmniAgent/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/VNDT1625/OmniAgent/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/VNDT1625/OmniAgent/releases/download
...
print_summary() {
    ...
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/VNDT1625/OmniAgent"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/VNDT1625/OmniAgent/issues"
    echo ""
